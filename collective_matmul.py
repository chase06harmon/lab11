...
def all_gather_matmul_pallas_scratch_specs(x):
    return {
        ...
        "gather_result": pltpu.VMEM(
            shape=(x.shape[0], 4 * x.shape[1]), dtype=jnp.bfloat16
        ),
    }
...
def all_gather_matmul_pallas_kernel(x_ref, w1_ref, out_ref, scratch_refs):
    ...
    * out_ref: [N_BATCH, K2]
    ...
    K1 = w1_ref.shape[0]
    K2 = w1_ref.shape[1]
    gather_ref = scratch_refs["gather_result"]
    ...
    left_src_ref = x_ref.at[pl.ds(0, N_BATCH), pl.ds(0, half)]
    left_dst_ref = gather_ref.at[pl.ds(0, N_BATCH), pl.ds(X_CHUNK * dev_id, half)]
    pallas_rdma_start(
    ...
    right_src_ref = x_ref.at[pl.ds(0, N_BATCH), pl.ds(half, half)]
    right_dst_ref = gather_ref.at[
        pl.ds(0, N_BATCH), pl.ds(X_CHUNK * dev_id + half, half)
    ]
    pallas_rdma_start(
    ...
    left_src_ref = x_ref.at[pl.ds(0, N_BATCH), pl.ds(half, half)]
    left_dst_ref = gather_ref.at[
        pl.ds(0, N_BATCH), pl.ds(X_CHUNK * dev_id + half, half)
    ]
    pallas_rdma_start(
        src_ref=left_src_ref,
    ...
    right_src_ref = x_ref.at[pl.ds(0, N_BATCH), pl.ds(0, half)]
    right_dst_ref = gather_ref.at[pl.ds(0, N_BATCH), pl.ds(X_CHUNK * dev_id, half)]
    pallas_rdma_start(
        src_ref=right_src_ref,
        dst_ref=right_dst_ref,
        dst_device_id=right_dev,
    ...
    pallas_rdma_wait_recv(
        dst_ref=gather_ref.at[pl.ds(0, N_BATCH), pl.ds(X_CHUNK * right_dev, half)],
    ...
    left_src_ref = gather_ref.at[pl.ds(0, N_BATCH), pl.ds(X_CHUNK * right_dev, half)]
    left_dst_ref = gather_ref.at[pl.ds(0, N_BATCH), pl.ds(X_CHUNK * right_dev, half)]
    pallas_rdma_start(
        src_ref=left_src_ref,
        dst_ref=left_dst_ref,
        dst_device_id=left_dev,
    ...
    pallas_rdma_wait_recv(
        dst_ref=gather_ref.at[
            pl.ds(0, N_BATCH), pl.ds(X_CHUNK * right_dev + half, half)
        ],
    ...
    pallas_rdma_wait_recv(
        dst_ref=gather_ref.at[pl.ds(0, N_BATCH), pl.ds(X_CHUNK * left_dev, half)],
    ...
    pallas_rdma_wait_recv(
        dst_ref=gather_ref.at[pl.ds(0, N_BATCH), pl.ds(right_dev * X_CHUNK, half)],
...
def all_gather_pallas_scratch_specs(x):
    return {
        ...
        "send_sem_half_2": pltpu.SemaphoreType.DMA(shape=(2,)),
        "recv_sem_half_2": pltpu.SemaphoreType.DMA(shape=(2,)),
        "gather_result": pltpu.VMEM(shape=(x.shape[0], x.shape[1]), dtype=jnp.bfloat16),
    }


def all_gather_pallas_kernel(x_ref, out_ref, scratch_refs):
    """
    Computes all_gather(x_ref), where the gather occurs along the second dimension.
...
def neural_network_pallas_scratch_specs(x, w1_refs, w2_refs):
    N_BATCH = x.shape[0]
    K1 = x.shape[1]
    K2 = w1_refs[0].shape[1]
    layers = w1_refs[0].shape[0]
    STAGE_1_SPLIT = get_split(x.shape[1])
    chunk_size = 1024 // N_DEVICES
    return {
        "x_accum": pltpu.VMEM(shape=(N_BATCH, K1), dtype=jnp.bfloat16),
        "all_gather_result": pltpu.VMEM(shape=(N_BATCH, K2), dtype=jnp.bfloat16),
        "all_gather": {
            i: {
                "send_sem_full_1": pltpu.SemaphoreType.DMA(shape=(2,)),
                "recv_sem_full_1": pltpu.SemaphoreType.DMA(shape=(2,)),
        ...
        "reduce_scatter": {
            i: {
                ...
                "send_sem_half_2": pltpu.SemaphoreType.DMA(shape=(STAGE_1_SPLIT,)),
                "recv_sem_half_2": pltpu.SemaphoreType.DMA(shape=(STAGE_1_SPLIT,)),
                "right_dest": pltpu.VMEM(
                    shape=(x.shape[0], chunk_size), dtype=jnp.bfloat16
                ),
                "left_dest": pltpu.VMEM(
                    shape=(x.shape[0], chunk_size), dtype=jnp.bfloat16
                ),
                "accross_right_dest": pltpu.VMEM(
                    shape=(x.shape[0], chunk_size // 2), dtype=jnp.bfloat16
        ...
        "ag_solo": all_gather_pallas_scratch_specs(x),
    }
...
def neural_network_pallas_kernel(init_x_ref, w1_refs, w2_refs, out_ref, scratch_refs):
    ...
    * out_ref: Pallas array reference with shape [N_BATCH, K1]. Should be written to.
    ...
    x_ref = scratch_refs["x_accum"]
    all_gather_results = scratch_refs["all_gather_result"]
    N_BATCH, K1 = init_x_ref.shape
    K1 = w1_refs[0].shape[0]
    dev_id = pallas_get_my_device_id()
    ag_scratch = scratch_refs["all_gather"]
    rs_scratch = scratch_refs["reduce_scatter"]

    chunk = K1 // N_DEVICES

    for i in range(w1_refs.shape[0]):
        w1 = w1_refs.at[i]
        w2 = w2_refs.at[i]
        if i == 0:
            all_gather_results[...] = jnp.astype(
                pl.dot(init_x_ref[...], w1[...]), jnp.bfloat16
            )

        else:
            x_inp = x_ref.at[pl.ds(0, N_BATCH), pl.ds(dev_id * chunk, chunk)]
            all_gather_matmul_pallas_kernel(
                x_inp, w1, all_gather_results, ag_scratch[(i - 1) % 2]
            )

        all_gather_results[...] = jax.nn.gelu(all_gather_results[...])

        # reduce_scatter_scratch = scratch_refs["reduce_scatter"][i]
        x_out = x_ref.at[pl.ds(0, N_BATCH), pl.ds(dev_id * chunk, chunk)]
        matmul_reduce_scatter_pallas_kernel(
            all_gather_results,
            w2,
            x_out,
            rs_scratch[i % 2],
    ...
    all_gather_pallas_kernel(
        x_out,  # N, K1 // N_DEVICE
        out_ref,  #
        scratch_refs["ag_solo"],
    )
